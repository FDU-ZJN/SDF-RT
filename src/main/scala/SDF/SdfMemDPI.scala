...
class SdfMem2R(
  ...
  GlobalConfig.memImplMode match {
    case 0 =>
      ...
      impl.io.wr_data := io.wr.wr_data
    case 1 =>
      val mem0 = Module(new SdfMemResourceBB(addrWidth, dataWidth))
      val mem1 = Module(new SdfMemResourceBB(addrWidth, dataWidth))
      val mems = Seq(mem0, mem1)
      for (i <- 0 until 2) {
        mems(i).io.clk := io.clk
        mems(i).io.reset := io.reset
        mems(i).io.globalIdx := io.globalIdx(i)
        mems(i).io.localIdx := io.localIdx(i)
        mems(i).io.en := io.en(i)
        io.data(i) := mems(i).io.data
        io.valid(i) := mems(i).io.valid
        mems(i).io.wr_en := io.wr.wr_en
        mems(i).io.wr_addr := io.wr.wr_addr
        mems(i).io.wr_data := io.wr.wr_data
      }
    case 2 =>
