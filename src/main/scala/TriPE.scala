...
//  val blockSizeBytes = (c.numPEs * 108).U
//  io.mem_req.bits  := current_batch.base_addr + (block_offset * blockSizeBytes)
//
...
//
//  io.mem_resp.ready := true.B // 简单处理：假设 PE 总是能接受
//
...
//  val outgoing_count = PopCount(pes.map(_.io.out_valid).asUInt)
//
...
//    val cmp = Module(new FCOMP(c.cfg))
//    cmp.io.a := a
//    cmp.io.b := b
//    Mux(cmp.io.a_lt_b, a, b)
//  })
//
