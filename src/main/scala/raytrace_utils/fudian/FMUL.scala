...
import raytrace_utils.fudian.utils.CLZ
...
class FMUL(cfg: FloatConfig = FloatConfig.FP32) extends Module {
  ...
  to_fadd_comb.fp_prod.exp  := Mux(hasZero, 0.U, exp_pre_round)
  to_fadd_comb.fp_prod.sig  := Mux(hasZero, 0.U, sig_shifted.tail(1).head(2 * precision - 1) | sig_shifted.tail(2 * precision).orR)
  to_fadd_comb.inter_flags.isInv    := special_iv
