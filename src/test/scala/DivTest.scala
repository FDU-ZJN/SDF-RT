...
class FRQTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  "FRQ" should "match precision and latency" in {
    // 使用默认配置，假设 fdivLatency = 6
    test(new FRQ(FloatConfig.FP32)) { c =>
  ...
  it should "handle continuous pipeline" in {
    test(new FRQ(FloatConfig.FP32)) { c =>
      // 测试流水线满载情况
      val inputs = Seq(2.0f, 3.0f, 4.0f)
      val outputs = inputs.map(1.0f / _)

      // 连续输入 3 拍
      inputs.foreach { b =>
        c.io.in_valid.poke(true.B)
        c.io.in.poke(fToHex(b).U)
        c.clock.step(1)
      }
      c.io.in_valid.poke(false.B)
