...
class BunnyNestedVoxelRenderer:
    def __init__(self, obj_path, cache_path="bunny_sdf_cache_v5.npz"):
        ...
        self.tri_centers = self.mesh.vertices[self.mesh.faces].mean(axis=1)

        self._build_triangle_bins()

        global _worker_mesh
        _worker_mesh = self.mesh
        if os.path.exists(self.cache_path):
            self.load_sdf(self.cache_path)
        else:
            self._build_all_sdf_parallel()

    def _build_triangle_bins(self):
        print("正在构建三角形空间分桶索引...")
        self.voxel_to_tris = {}
        tri_mins_idx = np.floor((self.tri_mins - self.bounds_min) / self.global_cell_size).astype(int)
        tri_maxs_idx = np.floor((self.tri_maxs - self.bounds_min) / self.global_cell_size).astype(int)
        tri_mins_idx = np.clip(tri_mins_idx, 0, GLOBAL_GRID_RES - 2)
        tri_maxs_idx = np.clip(tri_maxs_idx, 0, GLOBAL_GRID_RES - 2)

        for idx in range(len(self.mesh.faces)):
            mi, ma = tri_mins_idx[idx], tri_maxs_idx[idx]
            for i in range(mi[0], ma[0] + 1):
                for j in range(mi[1], ma[1] + 1):
                    for k in range(mi[2], ma[2] + 1):
                        key = (i, j, k)
                        if key not in self.voxel_to_tris:
                            self.voxel_to_tris[key] = []
                        self.voxel_to_tris[key].append(idx)
        print(f"分桶完成，活动单元格: {len(self.voxel_to_tris)}")

    def _build_all_sdf_parallel(self):
        print("未发现有效缓存，开始并行计算 SDF...")
        ...
        for i in range(GLOBAL_GRID_RES - 1):
            for j in range(GLOBAL_GRID_RES - 1):
                for k in range(GLOBAL_GRID_RES - 1):
                    if -0.15 < self.global_sdf_data[i, j, k] < 0.2:
                        active_indices.append((i, j, k))
        
        self.local_sdfs = {}
        total_cells = len(active_indices)
        num_cpus = min(os.cpu_count(), 32)
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_cpus) as executor:
            ...
            for count, future in enumerate(concurrent.futures.as_completed(futures), 1):
                ...
                if count % 50 == 0 or count == total_cells:
                    print(f"\rSDF 构建进度: [{count}/{total_cells}] {count/total_cells*100:.1f}%", end="")
        print("\nSDF 计算完成，正在保存...")
    ...
    def load_sdf(self, path):
        print(f"从缓存加载 SDF: {path}")
        data = np.load(path)
        self.global_sdf_data = data['global_sdf']
        keys = data['local_keys']; values = data['local_values']
    ...
    def _trilinear_interpolate(self, grid, local_p):
        ...
        v011 = grid[i0[0], i1[1], i1[2]]; v111 = grid[i1[0], i1[1], i1[2]]
    ...
    def _get_sdf_at(self, p):
        ...
        if np.any(rel_p < 0) or np.any(rel_p >= self.grid_size): return 0.5
        ...
        return self.global_sdf_data[g_tuple[0], g_tuple[1], g_tuple[2]]

    def _get_sdf_gradient(self, p):
        eps = 1e-3
        dx = self._get_sdf_at(p + [eps, 0, 0]) - self._get_sdf_at(p - [eps, 0, 0])
        dy = self._get_sdf_at(p + [0, eps, 0]) - self._get_sdf_at(p - [0, eps, 0])
        dz = self._get_sdf_at(p + [0, 0, eps]) - self._get_sdf_at(p - [0, 0, eps])
        grad = np.array([dx, dy, dz])
        return grad / (np.linalg.norm(grad) + 1e-8)

    def Fast_RT_get_hit(self, ro, rd, p_approx):
        # actual_idx, index_ray, locations = self.ray_intersector.intersects_id(
        #     ray_origins=[ro], 
        #     ray_directions=[rd], 
        #     multiple_hits=False
        # )
        rel_p = p_approx - self.bounds_min
        g_idx = tuple(np.clip((rel_p / self.global_cell_size).astype(int), 0, GLOBAL_GRID_RES - 2))
        candidate_indices = self.voxel_to_tris.get(g_idx, [])
        if not candidate_indices: return None, None
        
        normal_approx = self._get_sdf_gradient(p_approx)
        filtered_indices = [idx for idx in candidate_indices if abs(np.dot(self.tri_centers[idx] - p_approx, normal_approx)) < TRIHOLD]
        if not filtered_indices: filtered_indices = candidate_indices

        hit_tri, _, locations = trimesh.ray.ray_triangle.ray_triangle_id(
            triangles=self.mesh.triangles[filtered_indices], ray_origins=[ro], ray_directions=[rd], multiple_hits=False
        )
        if len(locations) > 0:
            actual_idx = filtered_indices[hit_tri[0]]
            return locations[0], self.mesh.face_normals[actual_idx]
        return None, None
    ...
    def get_hit_info(self, ro, rd):
        ...
        for _ in range(MAX_STEPS):
            ...
            if abs(r_curr)< THRESHOLD:
                p_hit = ro + rd * t
                h_p, norm = self.Fast_RT_get_hit(p_hit, rd, p_hit-offset, p_hit+offset)
    ...
    def render_parallel(self, save_name="render_fast_rt.png"):
        w, h = RENDER_RES
        image = np.zeros((h, w, 3))
        print(f"开始渲染 {w}x{h} 分辨率...")
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                done_rows += 1
                # 打印进度
                if done_rows % 5 == 0 or done_rows == h:
                    percent = (done_rows / h) * 100
                    elapsed = time.time() - start_time
                    avg_time = elapsed / done_rows
                    remain = avg_time * (h - done_rows)
                    print(f"\r渲染进度: [{done_rows}/{h}] {percent:.1f}% | 已用: {elapsed:.1f}s | 预计剩余: {remain:.1f}s", end="")
        
        print(f"\n渲染完成! 总耗时: {time.time()-start_time:.2f}s")
        plt.imsave(save_name, image)
        plt.imshow(image); plt.axis('off'); plt.show()

if __name__ == "__main__":
    # 请确保目录下有 bunny_10k.obj 或修改路径
