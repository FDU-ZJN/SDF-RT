...
class BunnyNestedVoxelRenderer:
    def __init__(self, obj_path, cache_path="shouban_cache_v1.npz"):
        ...
        self.bvh_nodes = None 
        self.bvh_root_offsets = None
        self.tri_indices_flat = None
    ...
    def _build_sub_bvh(self, tri_indices, tris_coords):
        nodes = []
        # 常数设置
        C_AABB = 1.0  # 遍历 AABB 节点的代价
        C_TRI = 1.5   # 相交三角形的代价
        NUM_BINS = 12 # 采样桶数量
        ...
        def build_recursive(indices):
            # 1. 计算当前集合的 AABB
            current_tris = tris_coords[indices] # shape: (N, 3, 3)
            v_all = current_tris.reshape(-1, 3)
            b_min, b_max = v_all.min(axis=0), v_all.max(axis=0)
            
            node_idx = len(nodes)
            # 占位：[min_x, y, z, max_x, y, z, lchild, rchild, count]
            nodes.append([*b_min, *b_max, 0, 0, 0])
            
            n_tris = len(indices)
            parent_area = get_surface_area(b_min, b_max)
            ...
            # 2. SAH 扫描寻找最优分裂轴和分裂点
            if n_tris > 1: # 只有三角形数量大于 1 才尝试分裂
                centers = current_tris.mean(axis=1) # 每个三角形的质心
                
                for axis in range(3):
                    axis_min = b_min[axis]
                    axis_max = b_max[axis]
                    if axis_max - axis_min < 1e-6: continue
                    
                    # 产生分桶采样点
                    for b in range(1, NUM_BINS):
                        split_val = axis_min + (axis_max - axis_min) * (b / NUM_BINS)
                        
                        left_mask = centers[:, axis] <= split_val
                        right_mask = ~left_mask
                        
                        n_left = np.sum(left_mask)
                        n_right = n_tris - n_left
                        
                        if n_left == 0 or n_right == 0: continue
                        
                        # 计算左右子集的范围
                        l_tris = current_tris[left_mask].reshape(-1, 3)
                        r_tris = current_tris[right_mask].reshape(-1, 3)
                        
                        area_l = get_surface_area(l_tris.min(axis=0), l_tris.max(axis=0))
                        area_r = get_surface_area(r_tris.min(axis=0), r_tris.max(axis=0))
                        
                        # SAH 公式: Cost = 2 * C_node + (AreaL/AreaP * Nl + AreaR/AreaP * Nr) * C_tri
                        current_cost = 2.0 * C_AABB + (area_l / parent_area * n_left + area_r / parent_area * n_right) * C_TRI
            ...
            # 3. 判定：是否应该作为叶子节点
            # 如果最优代价依然没有直接做叶子划算，或者无法分裂
            if best_axis == -1:
                nodes[node_idx][6] = -1 # leftchild = -1 表示叶子
                nodes[node_idx][7] = 0  # 此时 rchild 无意义
                nodes[node_idx][8] = n_tris
                return node_idx, indices

            # 4. 执行最优分裂
            centers = current_tris.mean(axis=1)
            left_indices = [indices[i] for i, c in enumerate(centers[:, best_axis]) if c <= best_split_val]
            right_indices = [indices[i] for i, c in enumerate(centers[:, best_axis]) if c > best_split_val]

            # 递归构建
            l_node, l_tris_sorted = build_recursive(left_indices)
            r_node, r_tris_sorted = build_recursive(right_indices)
            
            # 更新当前内部节点信息
            nodes[node_idx][6] = l_node
            nodes[node_idx][7] = r_node
            nodes[node_idx][8] = 0 # 内部节点三角形计数为 0
            
            return node_idx, l_tris_sorted + r_tris_sorted

        root_idx, sorted_tris = build_recursive(tri_indices)
        return nodes, sorted_tris
    ...
    def _build_nested_triangle_grid(self):
        ...
        all_bvh_nodes = []
        all_sorted_indices = []
        root_offsets_np = np.full((num_active, SUB_RES, SUB_RES, SUB_RES), -1, dtype=np.int32)
        
        tris_all_coords = self.mesh.triangles
        
        print("正在为活跃单元构建局部 BVH...")
        for i in range(num_active):
            for si, sj, sk in np.ndindex((SUB_RES, SUB_RES, SUB_RES)):
                tri_list = temp_grid.get((i, si, sj, sk), [])
                if len(tri_list) == 0: continue
                
                # 构建 BVH
                sub_nodes, sorted_tris = self._build_sub_bvh(tri_list, tris_all_coords)
                
                # 将该 Sub-grid 的 BVH 压入全局大表
                current_bvh_base = len(all_bvh_nodes) // BVH_NODE_SIZE
                root_offsets_np[i, si, sj, sk] = current_bvh_base
                
                current_tri_base = len(all_sorted_indices)
                
                # 修正节点中的索引偏移
                for n in range(len(sub_nodes)):
                    if sub_nodes[n][8] > 0: # 叶子
                        sub_nodes[n][6] = current_tri_base 
                        sub_nodes[n][7] = 0
                        current_tri_base += int(sub_nodes[n][8])
                    else: # 内部节点，修正子节点相对偏移
                        sub_nodes[n][6] += current_bvh_base
                        sub_nodes[n][7] += current_bvh_base
                
                all_bvh_nodes.extend(np.array(sub_nodes).flatten())
                all_sorted_indices.extend(sorted_tris)

        # 上传到 Taichi
        self.bvh_nodes = ti.field(ti.f32, shape=len(all_bvh_nodes))
        self.bvh_nodes.from_numpy(np.array(all_bvh_nodes, dtype=np.float32))
        self.bvh_root_offsets = ti.field(ti.i32, shape=(num_active, SUB_RES, SUB_RES, SUB_RES))
        self.bvh_root_offsets.from_numpy(root_offsets_np)
        self.tri_indices_flat.from_numpy(np.array(all_sorted_indices, dtype=np.int32))
    ...
    @ti.func
    def traverse_sub_bvh(self, ro, rd, root_ptr, min_t_in, hit_found_in, hit_norm_in):
        """在 Sub-grid 内部遍历 BVH 小树，实现严格的 Front-to-Back 排序与剪枝"""
        curr_min_t = min_t_in
        curr_hit_found = hit_found_in
        curr_hit_norm = hit_norm_in
        aabb_cnt, tri_cnt = 0, 0
        stack = ti.Vector([-1] * 12)
        stack[0] = root_ptr
        stack_ptr = 0 # 指向当前待处理的元素索引
        ...
        while stack_ptr >= 0:
            ...
            base = node_idx * BVH_NODE_SIZE
            # 这里的 AABB 测试用于处理刚刚从栈中弹出的节点
            # 因为 curr_min_t 可能在节点入栈后被更新（变小了）
            b_min = ti.Vector([self.bvh_nodes[base+0], self.bvh_nodes[base+1], self.bvh_nodes[base+2]])
            b_max = ti.Vector([self.bvh_nodes[base+3], self.bvh_nodes[base+4], self.bvh_nodes[base+5]])
            
            hit_aabb, t_box = self.aabb_intersect_dist(ro, rd, b_min, b_max)
            
            if not hit_aabb or t_box > curr_min_t:
                continue

            lchild = ti.cast(self.bvh_nodes[base+6], ti.i32);rchild = ti.cast(self.bvh_nodes[base+7], ti.i32);
            count = ti.cast(self.bvh_nodes[base+8], ti.i32)
            
            if count > 0:
                for k in range(count):
                    tri_idx = self.tri_indices_flat[lchild + k]
                    tri_cnt += 1
                    t, n = self.ray_triangle_intersect(ro, rd, tri_idx)
                    if 1e-4 < t < curr_min_t:
                        curr_min_t, curr_hit_norm, curr_hit_found = t, n, True
            
            else:
                idx_l = lchild
                idx_r = rchild
                l_base, r_base = idx_l * BVH_NODE_SIZE, idx_r * BVH_NODE_SIZE
                l_min = ti.Vector([self.bvh_nodes[l_base+0], self.bvh_nodes[l_base+1], self.bvh_nodes[l_base+2]])
                l_max = ti.Vector([self.bvh_nodes[l_base+3], self.bvh_nodes[l_base+4], self.bvh_nodes[l_base+5]])
                r_min = ti.Vector([self.bvh_nodes[r_base+0], self.bvh_nodes[r_base+1], self.bvh_nodes[r_base+2]])
                r_max = ti.Vector([self.bvh_nodes[r_base+3], self.bvh_nodes[r_base+4], self.bvh_nodes[r_base+5]])
                
                hit_l, tl = self.aabb_intersect_dist(ro, rd, l_min, l_max)
                hit_r, tr = self.aabb_intersect_dist(ro, rd, r_min, r_max)

                # 实时剪枝：距离大于当前最小值的盒子不入栈
                if hit_l and tl > curr_min_t: hit_l = False
                if hit_r and tr > curr_min_t: hit_r = False

                if hit_l and hit_r:
                    if tl < tr:
                        stack[stack_ptr + 1] = idx_r
                        stack[stack_ptr + 2] = idx_l
                        stack_ptr += 2
                    else:
                        stack[stack_ptr + 1] = idx_l
                        stack[stack_ptr + 2] = idx_r
                        stack_ptr += 2
                elif hit_l:
                    stack[stack_ptr + 1] = idx_l
                    stack_ptr += 1
        ...
        return curr_hit_found, curr_min_t, curr_hit_norm, tri_cnt
    @ti.func
    def nested_grid_search_hit(self, ro, rd):
        ...
        if t_far > 0 and t_near < t_far:
            ...
            for _ in range(1024):
                ...
                if active_idx >= 0:
                    bvh_root = self.bvh_root_offsets[active_idx, curr_s_idx.x, curr_s_idx.y, curr_s_idx.z]
                    if bvh_root >= 0:
                        hit_f, t_f, n_f, c_f = self.traverse_sub_bvh(ro, rd, bvh_root, min_t, hit_found, hit_norm)
                        tri_count += c_f
                        if hit_f:
                            min_t, hit_norm, hit_found = t_f, n_f, True
    ...
    @ti.func
    def get_hit_info(self, ro: ti.template(), rd: ti.template()):
...
if __name__ == "__main__":
    ...
    frame_dir = "frames/sdf_bvh"
    metrics_dir = "metrics/sdf_bvh"
