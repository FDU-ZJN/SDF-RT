...
def create_video(frame_dir, output_name, fps=24):
    ...
    def calculate_and_save_metrics(self):
        ...
        # 掩码：仅包含命中物体的像素
        active_mask = (hits_np > 0)
        num_active = np.sum(active_mask)
        num_total = steps_np.size

        # --- 1. SDF 步进次数统计 ---
        avg_steps_all = np.mean(steps_np)
        avg_steps_active = np.mean(steps_np[active_mask]) if num_active > 0 else 0

        # --- 2. 三角形求交次数统计 ---
        avg_tris_all = np.mean(tris_np)
        avg_tris_active = np.mean(tris_np[active_mask]) if num_active > 0 else 0

        # --- 打印结果 ---
        print("\n" + "="*40)
        print(f"渲染性能统计报告 ({RENDER_RES[0]}x{RENDER_RES[1]})")
        print("-" * 40)
        print(f"【SDF 步进次数】")
        print(f"  所有像素平均: {avg_steps_all:10.2f} 次/像素")
        print(f"  有效像素平均: {avg_steps_active:10.2f} 次/像素")
        print("-" * 40)
        print(f"【三角形求交测试】")
        print(f"  所有像素平均: {avg_tris_all:10.2f} 次/像素")
        print(f"  有效像素平均: {avg_tris_active:10.2f} 次/像素")
        print("-" * 40)
        print(f"命中像素占比: {(num_active/num_total)*100:.2f}%")
        print("="*40 + "\n")

        # 保存到本地文件
        output_dir = "metrics"
        os.makedirs(output_dir, exist_ok=True)
        with open(os.path.join(output_dir, "render_stats.txt"), "a") as f:
            f.write(f"{datetime.now()}: Steps(Act): {avg_steps_active:.2f}, Tris(Act): {avg_tris_active:.2f}\n")

        return {
            "avg_steps_active": avg_steps_active,
...
if __name__ == "__main__":
    ...
    create_video(frame_dir, "bunny_tachi_sdf_rt.mp4")
